...
content across every recipe and component. **Self-contained** — Katib does
not depend on the `/arabic` Claude Code skill (which is not available to
npm-installed users). This file is the authoritative source.

History: originally a snapshot from `/arabic`; carried forward verbatim
into v2 with light refreshes for the components / recipes vocabulary.

---

## House voice — الصوت الموحّد (متوافق مع قرار TeamSoul رقم ADR-040)

صوتٌ واحد يسري عبر كل أدوات جاسم — TeamSoul وKatib وكل مهارة تُنشئ نصًّا. مرتكزه:
**الإجابة أولًا · الإيجاز · المبنيّ للمعلوم · تسمية النتيجة · طلبٌ صريح**
(answer-first · short · active · outcome-named · ask-explicit). قواعد العلامة
والقواعد الأساسية أدناه هي هذا المرتكز، مضبوطةً للوثائق المطبوعة ثنائية اللغة؛
وملاحظات كل عائلة هي خريطة السطح الخاصة بنوع الوثيقة، فوق المرتكز.

المرجع الحاكم المشترك: قرار TeamSoul رقم **`ADR-040`** ومهارة `tsoul`. ونسخة Katib
أغنى عمدًا: تضيف سجلّ كل عائلة، وأعراف التوجيه EN↔AR، ومدقّق `content_lint.py` مع
بوابة الجودة الخماسية. **حين تُعدّل الصوت هنا، أبقِ المرتكز نفسه حتى لا يتباعد
الصوتان.** هذا الملف مستقلّ بذاته — يُسمّي المرتكز ولا يستورده.
...
e.g. `business-proposal-proposal`, `tutorial-how-to`, `personal-cv`, `legal-mou`.
Apply the notes for the family that matches the recipe you're authoring.

### Business proposal — formal (AR)
...
### Formal — NOC / authority-letter / government-letter / circular (AR)
...
### Financial — invoice / quote / statement / summary (AR)
...
- **ترقيم البنود متسلسل ومتدرّج.** `١.`، `١٫١`، `١٫٢٫١`. يشير الأطراف إلى البنود بأرقامها خلال التفاوض؛ إعادة الترقيم بعد التعديل انضباط لا إزعاج. مكوّن `clause-list` في «كاتب» يُرقّم تلقائياً — دعه يعمل.
...
- **مذكّرات التفاهم تُشير بوضوح إلى البنود المُلزِمة وغير المُلزِمة.** يعامل الأطراف عادةً المذكّرة كاملةً غير مُلزِمة باستثناء ثلاثة: السرّية، القانون الحاكم، النفقات. علّم كلّ بند مُلزِم بشكل صريح — وصفة «كاتب» للـ MoU تستخدم علامة `(مُلزِم)` بجانب تلك العناوين.
...
Katib is self-sufficient. Before any Arabic content leaves the skill, apply
this gate **explicitly**. It replaces the implicit dependency on the
`/arabic` Claude Code skill — which is unavailable to npm-installed users
and therefore not part of katib v2's contract.

### Step 1 · Pre-write (before first character)

1. **Read the recipe-specific notes section above** for the recipe family you're authoring (business-proposal, tutorial, personal, formal, report, academic, financial, editorial, marketing-print, legal). Each has non-negotiable conventions.
2. **Confirm audience register.** Recipe family default is formality 3-4 unless the family notes say otherwise. Match it.
...
- `core/content_lint.py` — mechanical-layer static checks (banned openers,
  emphasis crutches, jargon inflation, untranslated abbreviations,
  واو-chain runaways). The 5-dimension human-judgement score above sits on
  top of this lint.
- `references/writing.en.md` — the EN counterpart to this gate.
- `recipes/*.yaml` — per-recipe templates whose `inputs_by_lang.ar` blocks
  must satisfy the rules above.
- `components/*/ar.html` — RTL-aware bilingual templates.

The `/arabic` Claude Code skill (upstream) may still be used when producing
**free-form** content (LinkedIn, email, articles outside of Katib) inside
Claude Code itself. For Katib renders, the rules above are the complete
contract — and they ship with every npm install.
